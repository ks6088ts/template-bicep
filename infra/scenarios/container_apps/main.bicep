...
@description('Container image to use for the Container App')
...
@description('Optional. Array of existing User Assigned Managed Identities (UAMI) to attach to the Container App and grant roles to. Defaults to an empty array.')
param existingUserAssignedIdentities uamiReference[] = []

@description('Optional. Array of object (principal) IDs of existing Microsoft Entra service principals to grant roles to at Container App scope. Defaults to an empty array.')
param existingServicePrincipalObjectIds string[] = []

@description('Optional. Array of object IDs of existing Microsoft Entra users to grant roles to at Container App scope. Defaults to an empty array.')
...
// Grant AcrPull to the created UAMI
module acrPullRoleAssignment '../../modules/role_assignment/main.bicep' = {
  ...
  params: {
    ...
    roleAssignmentNameSeed: guid(containerRegistry.outputs.id, userAssignedIdentity.outputs.principalId, acrPullRoleDefinitionId)
    principalType: 'ServicePrincipal'
  }
}

// Get the Log Analytics workspace keys
resource lawResource 'Microsoft.OperationalInsights/workspaces@2023-09-01' existing = {
  scope: az.resourceGroup(resourceGroupName)
  #disable-next-line BCP334
  name: logAnalyticsWorkspaceName
}
...
module containerAppEnvironment '../../modules/container_app_environment/main.bicep' = {
  ...
  params: {
    ...
    location: location
    tags: tags
    logAnalyticsWorkspaceCustomerId: lawResource.properties.customerId
    logAnalyticsWorkspaceSharedKey: lawResource.listKeys().primarySharedKey
    zoneRedundant: false
  }
  dependsOn: [
...
// Prepare the user-assigned identity resource IDs for the Container App
var existingUamiIds = [
  for i in range(0, length(existingUserAssignedIdentities)): existingUamis[i].id
]
var containerAppUserAssignedIdentityIds = concat([userAssignedIdentity.outputs.id], existingUamiIds)
...
module containerApp '../../modules/container_app/main.bicep' = {
  ...
  params: {
    ...
    userAssignedIdentityResourceIds: containerAppUserAssignedIdentityIds
...
// Role assignments for existing UAMIs at Container App scope
module uamiRoleAssignments '../../modules/role_assignment/main.bicep' = [
  for (pair, i) in uamiRolePairs: {
    name: take('${name}-uami-ca-role-${i}-deployment', 64)
    scope: az.resourceGroup(resourceGroupName)
    params: {
      #disable-next-line BCP334
...
module servicePrincipalRoleAssignments '../../modules/role_assignment/main.bicep' = [
  for (pair, i) in servicePrincipalRolePairs: {
    name: take('${name}-sp-ca-role-${i}-deployment', 64)
    scope: az.resourceGroup(resourceGroupName)
    params: {
      #disable-next-line BCP334
      targetContainerAppName: containerAppName
...
module userRoleAssignments '../../modules/role_assignment/main.bicep' = [
  for (pair, i) in userRolePairs: {
    name: take('${name}-user-ca-role-${i}-deployment', 64)
    scope: az.resourceGroup(resourceGroupName)
    params: {
      #disable-next-line BCP334
      targetContainerAppName: containerAppName
...
@description('The HTTPS URL of the Container App')
output containerAppUrl string = 'https://${containerApp.outputs.fqdn}'

@description('The resource ID of the AcrPull role assignment for the created UAMI')
...
output uamiRoleAssignmentIds string[] = [
  for (pair, i) in uamiRolePairs: uamiRoleAssignments[i].outputs.id
]
