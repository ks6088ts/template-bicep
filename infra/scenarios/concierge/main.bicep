...
@description('Enable Application Insights and Log Analytics workspace for observability. When true, creates workspace, Application Insights, Foundry diagnostic settings, and Foundry tracing connection.')
param enableApplicationInsights bool = true

@description('Enable PostgreSQL Flexible Server with pgvector support. When true, creates PostgreSQL server with Entra ID-only authentication.')
...
@description('Microsoft Entra ID administrator for the PostgreSQL Flexible Server. When omitted, the principal executing the deployment (returned by deployer()) is registered as the administrator so the scenario can be deployed without any manual edits. Only used when enablePostgresql is true.')
...
@description('The compute SKU name for the PostgreSQL flexible server')
...
@description('Firewall rules to create on the PostgreSQL flexible server. Defaults to an "Allow Azure services" rule (0.0.0.0/0.0.0.0).')
...
@description('Databases to create on the PostgreSQL flexible server. Defaults to a single "appdb" database for validation.')
...
module foundryDiagnosticSettings '../../modules/diagnostic_settings/main.bicep' = if (enableApplicationInsights) {
  name: take('${name}-foundry-diag-deployment', 64)
  scope: az.resourceGroup(resourceGroupName)
  params: {
